...
Public Class Form1
    ...
    Private Async Sub Timer1_Tick(sender As Object, e As EventArgs) Handles Timer1.Tick
        Await Task.Run(Sub()
                           '転送1
                           Dim di1 As New System.IO.DirectoryInfo(kannsi)
                           ...
                           '転送2
                           Dim di2 As New System.IO.DirectoryInfo(kannsi)
    ...
    Private Sub 転送処理1(ByVal s As String)
        ...
        If kakutyousi <> ".tmp" Then
                    ...
                    If yousosuu >= 4 Then '配列の要素数チェック----要素数4以上
                        ...
                        Try '-----------------------------------------------------------------------------------------ファイルのコピー処理
                            If FileOpenCheck(s) = True Then
                                System.IO.File.Copy(s, tennsousaki & "\" & fname, True)
                            Else
                                flg = False
                                errorstr = "転送タイムアウト"
                            End If
            ...
            If flg = False Then '---------------------------------------------------------------------------------------------------エラー時処理
                ...
                If FileOpenCheck(s) = True Then
                    System.IO.File.Copy(s, tennsoumoto & "\" & fname, False) 'エラーコードを付けて転送元へ返信
                End If
    ...
    Private Sub 転送処理2(ByVal s As String)
        ...
        If FileOpenCheck(s) = True Then
            System.IO.File.Copy(s, tennsoumoto & "\" & fname, False)
        End If
        If FileOpenCheck(s) = True Then
